    ...
    <div className="space-y-4 animate-slide-up">
      ...
      {loadingGoals ? (
        ...
        <div className="p-8 bg-white border border-slate-100 rounded-[20px] text-center">
          ...
          <p className="text-xs text-slate-400 mt-0.5">Buat target untuk laptop baru, liburan, atau dana darurat.</p>
        </div>
      ) : (
        <div className="space-y-3.5">
          {goals.map(g => {
            ...
            return (
              <div key={g.id} className="p-4 bg-white border border-slate-100 rounded-[22px] space-y-3.5 shadow-2xs relative">
                <div className="flex justify-between items-start gap-2">
                  <div className="min-w-0">
                    <h3 className="text-xs font-extrabold text-slate-700 truncate">{g.title}</h3>
                    <span className="text-[11px] text-slate-400 block mt-0.5">
                      Terkumpul: <span className="font-extrabold text-slate-600">{formatRupiah(g.currentAmount)}</span>
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <button
                      ...
                      className="px-2.5 py-1 rounded bg-emerald-50 text-emerald-600 hover:bg-emerald-100 text-[11px] font-bold transition-colors cursor-pointer"
                    >
                      Tabung (+)
                    </button>
                    <button
                      ...
                      className="px-2.5 py-1 rounded bg-rose-50 text-rose-500 hover:bg-rose-100 text-[11px] font-bold transition-colors cursor-pointer"
                    >
                      Tarik (-)
                    </button>
                    <button
                      ...
                      className="p-1.5 text-slate-350 hover:text-rose-500 active:scale-95 transition-colors cursor-pointer"
                      title="Hapus"
                    >
                      <Trash2 size={13} />
                    </button>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex justify-between text-[10px] font-bold text-slate-400 tracking-wide">
                ...
                {percent >= 100 && (
                  <div className="bg-emerald-50 border border-emerald-100 text-emerald-600 p-2.5 rounded-xl text-xs font-bold flex items-center gap-1.5">
